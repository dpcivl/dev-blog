...
- **Solution**: [`src/components/Feedback.astro`](https://github.com/dpcivl/dev-blog/blob/main/src/components/Feedback.astro) — a visible dashed-border box at the bottom of every post. The email address is shown inline as text, followed by three parallel actions. **① Copy address** (Clipboard API + "Copied!" visual feedback, with a text-selection fallback when the API is unavailable) — covers Korean users who mostly use Naver/Kakao mail. **② Open in Gmail** (Gmail web compose URL, subject pre-filled) — targets the developer majority. **③ Open a GitHub Issue** (title pre-filled). i18n aware (KO/EN copy branches). `mailto:` was intentionally dropped because Korean users rarely use it.
