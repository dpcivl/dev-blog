---
title: "퀀트 공부 00 — pandas 기초 · 추세추종 vs 평균회귀 · 백테스트 4대 편향 · 가짜 알파"
description: "퀀트 학습 시리즈 시작. pandas Series/DataFrame, loc/iloc, 워밍업 구간(NaN), 수정주가로 인한 조용한 데이터 오염, 추세추종(MA·골든/데드 크로스) vs 평균회귀, 백테스트의 4대 편향(미래참조·과최적화·비현실적 거래비용·생존편향), 그리고 한 점이 아니라 분포로 봐야 가짜 알파를 거른다 (B&H 초과 비율 / Alpha 중앙값 / 노출시간 / 워크포워드)."
...
> **"이 전략이 B&H 를 이겼나"** (한 점) 가 아니라
> **"여러 조건에서 돌렸을 때 알파 중앙값이 양수인가, 노출시간 대비 합리적인가"** (분포)
...
> B&H 초과 비율은 화려한 숫자가 잘 나오는데, **노출시간이 짧고 Alpha 중앙값이 음수** 인 케이스가 의외로 많다. "이긴 적이 많다" 와 "꾸준히 이긴다" 는 다른 얘기.
