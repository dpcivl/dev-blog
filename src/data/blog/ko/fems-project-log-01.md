...
새 시리즈를 시작한다. **FEMS (Factory Energy Management System)** 프로젝트 일지다.

이전에 회사에서 엣지 AI 보드 위에서 객체 감지를 돌려본 경험이 있어서 **공장 / 산업 데이터 + AI 의 결합** 이라는 흐름이 계속 머릿속에 남아 있었다. 그 연장선에서 **FEMS 도메인을 학습하면서 RAG 프로토타입을 한 번 깎아보기로** 결정했다. "공부했습니다" 보다 "이 정도까지 돌려봤습니다" 가 손에 더 잘 붙는다.
...
RAG · 임베딩 · 벡터 DB 는 최근 [RAG 직접 구현](/posts/rag-from-scratch-embedding-and-similarity-search) / [RAG 시스템 구축](/posts/rag-system-chroma-blog-qa) 에서 다뤄봤기 때문에 그나마 친숙하다. **로컬 LLM (Ollama)** 이 새로 추가되는 축이다.
...
로컬 LLM 의 가성비/속도는 **VRAM** 이 가른다. **VRAM 6GB 는 큰 모델이 안 돌아간다.** 7B 모델도 양자화(4bit) 안 하면 빠듯하다. 13B 이상은 사실상 불가능하다.
...
오늘은 일단 **테스트용 가이드라인을 직접 작성** 해서 RAG 동작부터 검증한다.
...
3-provider 비교를 위해 각각의 모델을 다음과 같이 골랐다:
...
로컬은 **VRAM 이 모델 선택을 가른다.** 7B 모델은 양자화(4bit) 거치면 VRAM 4–5GB 정도라 6GB 환경에 들어간다. 13B 이상은 시도조차 불가능하다. **qwen2.5** 는 7B 라인에서 한국어/추론 성능이 균형 잡혀서 채택했다.

> 클라우드 쪽은 일부러 **상위 (Opus 4.8) vs 중간 (gpt-4o)** 으로 가격·품질 스펙트럼을 갖다 놨다. 이래야 "로컬이 어디까지 따라잡나" 를 두 기준으로 볼 수 있다.

### Smoke test — 첫 실행 (콜드 스타트)

세 provider 가 동일한 질문에 답하는지부터 확인했다:
...
- **Opus 4.8 이 gpt-4o 보다 약 5배 비싸다** ($0.00213 vs $0.00041)
- **ollama (qwen2.5:7b) 가 95초** — 콜드 스타트 때문이다. 첫 호출이 모델 로딩이라 대부분이 로딩 시간이다.

### Smoke test — 두 번째 실행 (워밍업 후)

이미 모델이 메모리에 올라온 상태로 다시 돌려봤다:
...
**95초 vs 10초** 의 차이는 운영적으로 매우 크다. 즉, **로컬 LLM 은 "처음 한 번" 을 어떻게 다루느냐** 가 latency 의 핵심이다:
...
임베딩은 **Ollama 의 `bge-m3`** 로 결정했다.

**선정 이유:**

- **한국어/다국어 우수** — FEMS 가이드라인이 한국어니까 핵심 조건이다
- **별도 임베딩 API 불필요** — Ollama 가 LLM 도 임베딩도 같이 준다
- **로컬 완결** — 외부 API 호출 없이 임베딩까지 처리된다

**검토했지만 안 쓴 것:**

| 옵션 | 안 쓴 이유 |
|---|---|
| LangChain / LlamaIndex | 의존성이 너무 무겁고 추상화가 과하다 |
| sentence-transformers (로컬) | torch 설치가 무겁고, Ollama 가 더 합리적이다 |

그리고 한 가지 주의 — **Chroma 의 기본 임베딩 함수는 영어 특화다.** 한국어 문서를 그대로 넣으면 임베딩 품질이 떨어진다. 그래서 **임베딩을 직접 계산해서 Chroma 에 주입** 하는 식으로 진행했다.

> 지난 [RAG 직접 구현 글](/posts/rag-from-scratch-embedding-and-similarity-search) 에서 OpenAI 임베딩으로 한국어를 돌렸을 때 유사도 절대값이 0.3–0.5 수준으로 낮게 나왔었다. bge-m3 는 다국어 특화라 이 점이 개선되는지가 이번 검증 포인트다.

## 인덱스 구축 — 청크 5개부터

테스트용 가이드라인을 **800자 이하로 청크 분할** 후 인덱싱했다:
...
특히 **두 번째 질문 (피크 수요)** 의 dist 0.272 는 굉장히 잘 매칭된 편이다. **bge-m3 가 한국어를 제대로 다루고 있다** 는 의미다.

> 이전 글에서 OpenAI `text-embedding-3-small` 로 한국어를 돌렸을 땐 1위 유사도가 0.3–0.5 사이였는데, bge-m3 는 0.27–0.39 (distance, 낮을수록 좋음 → 유사도로 환산 시 0.6–0.73) 로 나온다. 다국어 임베딩의 차이가 숫자로 드러난다.
...
이어서 실제 공개 데이터 PDF (한국에너지공단 FEMS 구축 가이드, 에너지절약 기술자료 등) 를 붙여서 청크 수를 키우고, **로컬 vs 클라우드 답변 품질을 진짜 도메인 질문으로 비교** 하는 그림으로 이어진다.
