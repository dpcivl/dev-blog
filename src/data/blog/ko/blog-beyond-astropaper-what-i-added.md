...
이 블로그는 **[AstroPaper](https://github.com/satnaing/astro-paper)** 를 기반으로 시작했지만, 그대로 두지 않았다. **매일 발행하는 1인 콘텐츠 시스템** 관점에서 반년 넘게 부딪히면서 얹은 기능들을 정리한다. 각 항목은 **"AstroPaper 기본 상태에서 뭐가 부족했나 → 어떻게 해결했나 → 결과"** 3단으로.
