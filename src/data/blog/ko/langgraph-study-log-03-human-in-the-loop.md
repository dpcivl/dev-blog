...
- **`thread_id` 가 같으면** 이전 대화를 계속한다 (체크포인트 이어 받기)
- **`thread_id` 가 다르면** 새 대화를 시작한다
...
3. 툴 호출 시 이름이 위험 리스트에 있으면 → **사용자 input 을 받는다**
...
2. **LLM 의 친절함이 결정론적 게이팅을 가로막을 수 있다** — 시스템 프롬프트의 모호한 안전 지시는 HITL 과 충돌한다
3. **운영 환경에서는 MemorySaver 대신 SqliteSaver / PostgreSaver** — 프로세스 재시작을 견뎌야 한다
