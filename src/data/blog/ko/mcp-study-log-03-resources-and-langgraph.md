...
[이전 LangGraph 시리즈](/posts/langgraph-study-log-01) 패턴 그대로인데 **`asyncio`** 가 추가됐다.
...
이게 [#1 에서 비교했던 "Tool Use vs MCP" 표](/posts/mcp-study-log-01#왜-mcp-가-필요한가--tool-use-의-한계) 의 **"범용 표준"** 항목이 실제로 어떻게 발현되는지를 직접 본 셈이다.
...
2. **MCP 통신은 비동기** — `ainvoke` 가 자연스러운 선택이다. 동기 코드만 쓰던 사람은 `asyncio` 한 번 짚고 가야 한다.
