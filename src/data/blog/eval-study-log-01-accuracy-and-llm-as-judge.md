...
- **개선폭 측정 불가능** — 얼마나 좋아졌는지 모름
- **트레이드오프 놓침** — 코드 수정으로 A 가 좋아지고 B 가 나빠진 케이스, 측정 안 하면 B 의 악화를 놓침
- **의사결정 근거 빈약** — 수치 없이 "감으로" 결정하면 최선의 결정이 안 됨
...
오늘은 **정확도 기반 + LLM-as-Judge** 두 가지 직접 구현.
...
> 임베디드 개발하던 시절 TDD 를 도입해보려고 연습했던 적이 있는데, **AI 의 품질을 검증하기 위한 테스트 케이스 작성도 같은 결의 작업** 이라는 느낌. 입력 → 기대 출력 페어를 미리 정의해두면 회귀 테스트 같이 굴릴 수 있음.
...
**개선폭이 숫자로 보인다.** 65% → 80% → 95% 같은 식. **이게 Eval 의 존재 이유.** 결과는 json 으로도 저장해서 나중에 비교 가능.
...
지금은 가짜 질문+답변 쌍이지만, 실전에서는 **실제 시스템이 생성한 답변** 을 그대로 평가에 넣게 됨.
...
사람이 보면 "전자가 더 낫다" 정도의 인상인데, **LLM 이 여러 시점에서 평가해서 점수 차이를 명시적으로** 만들어줌. 8 vs 3 → 5점 차이. 이 격차가 곧 개선 가능 폭.
...
2. **같은 답변을 두 번 평가했을 때 점수가 다를 수 있음** — 결정론적이지 않음
3. **같은 모델이 만든 답변을 후하게 평가** 하는 경향 (self-preference bias)

→ **그래서 사람 샘플 검증이 마지막 안전장치.** LLM 끼리만 평가하면 폐쇄 루프에 빠짐. 일부 케이스는 사람이 직접 보고 LLM 평가의 신뢰도 자체를 검증해야 함.
...
2. **정확도 기반 Eval 의 효용** — "description 다듬기로 65 → 80 → 95% 까지" 같은 개선 사이클이 숫자로 보임
3. **LLM-as-Judge 는 주관적 평가의 수치화 도구** — 다만 평가자 변동성 / 자기 모델 편향이 있어 사람 샘플 검증을 뺄 수 없음

본인이 만든 시스템의 능력을 **객관적으로 설명** 하려면 결국 수치 근거가 필요. 그게 없으면 "이 시스템 좋아요" 라고 말해도 듣는 사람 입장에선 검증할 방법이 없다. **Eval 의 존재 이유가 결국 그 객관적 설명 가능성** 이라고 정리됨.
