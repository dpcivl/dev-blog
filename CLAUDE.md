...
2. **포스트 작성** ([src/data/blog/ko/<slug>.md](src/data/blog/ko/))
   ...
   - 사용자가 거칠게 적은 메모를 다음 형태로 정돈한다:
     ...
     - 메모에 `+` · `?` · `!` 가 붙은 줄이 있으면 아래 "메모 표기 규약" 에 따라 처리
...
작성자가 인박스 · 스크래치 메모에서 줄 앞에 붙이는 기호다. 세 개 다 자주 쓰이니 발행 시 아래대로 처리한다.

| 기호 | 작성자 의도 | 발행 시 처리 |
| --- | --- | --- |
| `+` | 추가 팁 | **본문에 녹인다.** 관련 있는 대목 옆에 배치하거나, 팁이 여러 개면 별도 소절로 묶는다. 기호는 지운다 |
| `?` | 궁금한 점 (미해결) | **"더 공부해볼 것" 으로 옮긴다.** 본문에서 추측으로 답하지 않는다. 가능하면 공식 문서 링크를 붙여 학습 가능한 형태로 만든다 |
| `!` | 바로 위 `?` 에 대한 답 (해결됨) | **본문에 풀어쓴다.** 짝이 있으므로 "더 공부해볼 것" 으로 보내지 않는다 |

**핵심은 `?` 와 `!` 의 짝 판정이다.**

- `?` 만 있으면 → 미해결. "더 공부해볼 것" 행
- `?` 뒤에 `!` 가 붙어 있으면 → 해결됨. 본문에서 "이게 이상했다 → 이유는 이거였다" 흐름으로 서술
- 짝인 `?`/`!` 는 그 글에서 **제목 후보로 먼저 검토한다.** 작성자가 직접 막히고 직접 푼 지점이라 글의 중심인 경우가 많다

> 실제 사례: `?sales.loc["화":"목"] 얘는 왜 목요일까지 표출함?` + `!loc 의 기본 동작이 둘 다 포함하는 거라서..` → 제목이 「loc 는 끝을 포함하고 iloc 는 제외한다」가 됐다.

메모 본문에 기호가 붙은 줄이 여러 개 흩어져 있을 수 있으니 발행 전에 한 번 훑는다. 마크다운 이미지(`![alt](...)`)가 `!` 패턴에 걸리므로 제외한다.

```bash
grep -nE '^\s*[+?]|^\s*![^[]' <메모파일>
```

> **⚠️ 2026-09-01 이전 메모는 규약이 다르다.** 그때는 `!` 가 **팁**이었고 `?` 가 **검색해볼 것**이었다 (`항만도메인공부 - 1.md` 에 `!는 팁, ?는 검색해볼 거` 라고 직접 적혀 있다). 오래된 메모를 다룰 때는 `!` 를 답으로 단정하지 말고 앞에 짝이 되는 `?` 가 있는지 먼저 본다. 짝이 없는 단독 `!` 는 옛 규약의 팁일 가능성이 높다.
...
**IP 주소 (2026-09-06 실제 사고):** 작성자의 집 공인 IP 가 `deploy/README.md` 와 블로그 본문에 그대로 들어간 채 push 됐다. 하필 *"집 IP 는 공개 저장소에 올리지 말 것"* 이라고 적어둔 문서 자체였다. 지금은 `pnpm check:md` 가 공인 IP 를 잡아 exit 1 로 막는다 (사설 대역과 RFC 5737 문서용 주소는 통과). **예시가 필요하면 `192.0.2.1` · `198.51.100.1` · `203.0.113.1` 을 쓴다.**
...
3. 포스트 작성 (frontmatter, "공부할 것" 섹션 포함, **다이어그램 트리거 검토**, **`+`/`?`/`!` 표기 처리** — 위 "메모 표기 규약" 절 참고)
...
### 8. 작성자 원고 스타일 — 있으면 그대로 살린다

작성자가 메모 아래 `- - -` 구분선을 긋고 **직접 쓴 원고**를 붙여줄 때가 있다. 그 원고는 초안이 아니라 **어투 레퍼런스**다. 내용을 다시 쓰지 말고 아래만 바꾼다.

- **존댓말(`~습니다`) → `~다`.** 작성자는 원고를 존댓말로 쓰지만 블로그는 처음부터 `~다` 로 통일돼 있다. 어미만 바꾸고 문장 구조는 건드리지 않는다
- 사실관계가 틀린 곳만 정정한다 (아래 참고)
- 그 외에는 **문장을 다듬지 말 것.** 매끄럽게 만들려는 손질이 곧 AI 티다

**원고에서 관찰되는 특징 — 발행본에서도 유지한다:**

| 특징 | 예시 (2026-09-06 `prog-select-q3` 원고) |
| --- | --- |
| 실패한 시도를 먼저 적는다 | "당연히 안 되는 걸 알면서도 `FROM`을 두 개 써서 실행해봤습니다. 당연히 안됐구요." |
| 모르는 건 모른다고 쓴다 | "여기저기서 주워들은 걸로 풀 수 있었습니다" |
| 들은 지식은 출처를 흐리지 않는다 | "~라고 합니다", "~라고 들었다" — 단정하지 않는다 |
| 자기 답에 스스로 단서를 단다 | "테이블이 두 개라고 무작정 서브쿼리를 사용할 필요는 없습니다" |
| 소제목이 주제어가 아니라 **결론 문장** | "`FROM`은 한 쿼리당 하나씩만" (○) / "FROM 절에 대하여" (×) |
| 코드를 한 줄씩 짚어 설명 | 별칭 `F`·`I` 를 왜 뒀고 `ON` 이 무엇을 맞추는지 순서대로 |
| 마무리에 교훈을 붙이지 않는다 | 마지막 설명이 끝나면 그냥 끝난다 |

**정정은 하되 덮어쓰지 않는다.** 원고에 사실관계 오류가 있으면 작성자가 실제로 겪은 것은 남기고, 정확한 설명을 옆에 덧붙인다. 예) "`FROM` 은 한 쿼리당 하나" → 작성자가 막힌 건 `FROM` 절을 두 번 쓴 것이 맞으므로 그 경험은 그대로 두고, "`FROM A, B` 처럼 한 절에 테이블을 나열하는 건 된다" 를 아래에 붙인다.

**원고가 없는 날은** 평소대로 거친 메모를 풀어쓴다. 이 절은 원고가 딸려 왔을 때만 적용된다.

### 9. 초안에 없는 것은 지어내지 않는다

메모를 풀어쓰는 일은 **빈 곳을 메우는 일이 아니다.** 아래 셋은 특히 그럴듯하게 만들어지기 쉬우니, 쓰기 전에 초안에 있는지 먼저 확인한다.

| 항목 | 규칙 |
| --- | --- |
| **숫자 · 출력 · 에러** | 초안에 있는 값만 그대로 옮긴다. 반올림하거나 "약 5천 건" 으로 바꾸지 않는다. 초안에 없으면 **그 대목을 쓰지 않는다** |
| **한 줄 일반화 (교훈)** | 실수 뒤에 붙는 "그래서 이렇게 해야 한다" 는 작성자가 실제로 그렇게 말했을 때만 쓴다. 없으면 사실 서술에서 멈춘다 |
| **인과** | "A 때문에 B" 는 직접 확인했을 때만 단정한다. 확인 못 했으면 `~인 것 같다` · `확인은 필요하다` 로 남긴다 |

**빈 자리를 `[출력 붙일 것]` 같은 표시로 남기고 발행하지 않는다.** 그대로 배포된다. 자리가 비면 그 대목을 빼거나, 의문 형태로 "더 공부해볼 것" 으로 옮긴다.

**Why:** 작성자가 자기 글을 다시 읽으며 학습하는 게 이 블로그의 목적이다. 지어낸 교훈과 직접 얻은 교훈은 발행 시점에는 구분이 안 되지만, 나중에 그 글을 근거로 판단할 때 문제가 된다. 숫자도 한번 박히면 그게 기록이 된다.

**How to apply:**
- 확인이 필요한데 확인할 수단이 없으면 **발행 전에 작성자에게 묻는다.** 추측으로 채우고 넘어가지 않는다
- 내가 직접 확인해서 정정한 것은 **확인했다는 사실까지 본문에 적는다** (예: 「직접 확인해봤다」 뒤에 실행 결과)
- 반대로 초안에 있는 실수 · 오해 · 실패한 시도는 **빼지 않는다.** 그게 글의 몸통이다

> 실제 사례 (2026-09-06, `20260902-2`): 메모에 *"아무 것도 안 걸리기 때문에 문제 있는 데이터는 없다"* 라고 적혀 있었는데, 같은 메모 뒤쪽 요약표에는 30일 초과가 4838건이었다. 앞뒤가 어긋난 것을 그대로 옮겼으면 틀린 결론이 발행됐다. 조건식을 직접 돌려보고 정정했다.

### 10. `in-sight` 시리즈는 본문에 손대지 않는다

`in-sight` 태그가 붙은 글은 **작성자가 직접 쓰는 에세이**다. 다른 글과 역할 분담이 완전히 다르다.

| 항목 | 담당 |
| --- | --- |
| 제목 | Claude 가 제안 → 작성자가 확정 |
| frontmatter | Claude |
| **본문 — 문장 · 구조 · 소제목 · 어미** | **작성자 전부** |
| 발행 검사 (`check:md` · 빌드 · 이미지 변환) · 커밋 · 푸시 | Claude |

**Why:** 8번은 "원고가 오면 어미만 `~다` 로 바꾼다" 인데, **이 시리즈는 그 어미조차 바꾸면 안 된다.** 에세이는 문장의 리듬 자체가 내용이고, 작성자가 "제목만 도움받고 내용 · 형식 전부 수제로 쓰겠다" 고 명시한 시리즈다 (2026-09-22).

**How to apply:**
- 오타 · 맞춤법도 **고치지 말고 물어본다**
- **"더 공부해볼 것" 섹션을 붙이지 않는다.** 그건 학습 로그용 관례다
- 다이어그램을 먼저 제안하지 않는다. 작성자가 mermaid 블록을 넣었으면 렌더만 한다
- 발행 절차 2번의 "상황 → 시도/검증 → 원인/해결 → 회고" 흐름을 강요하지 않는다
- **보안 스크럽은 그대로 한다.** 시크릿 · 공인 IP 가 보이면 발행을 멈추고 알린다 — 이건 본문 수정이 아니라 발행 보류다
