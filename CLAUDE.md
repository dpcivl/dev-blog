...
2. **포스트 작성** ([src/data/blog/ko/<slug>.md](src/data/blog/ko/))
   ...
   - 사용자가 거칠게 적은 메모를 다음 형태로 정돈한다:
     ...
     - 구어체 한국어, 1인칭("나"), `~다` 어미
     - 코드 블록 / 스크린샷 / **다이어그램** 적극 활용
     - 📐 **다이어그램 트리거** — 아래 중 하나라도 해당하면 mermaid 를 넣을지 먼저 검토한다. 표와 글머리표로만 쓰지 말 것:
       - **3단계 이상의 순서·절차** (수출입 프로세스, 배포 파이프라인, 요청 흐름)
       - **계층·포함 관계** (시설 분류, 식별자 상하 관계, 폴더 구조)
       - **여러 주체가 주고받는 흐름** (기관·서비스 간 데이터 이동)
       - **분기와 상태 전이** (조건에 따라 갈리는 처리, 상태 머신)
       - 반대로 **정적인 목록·대비표는 표가 낫다.** 굳이 그리지 않는다.
...
6. **다이어그램 렌더** — 포스트에 ` ```mermaid ` 블록을 넣었다면 **커밋 전에 반드시 `pnpm mermaid:render`** 실행. Astro 에는 mermaid 플러그인이 없어서(아래 섹션 참고) 렌더를 잊으면 다이어그램 대신 **코드 블록이 그대로 발행된다.** 빌드도 링크 검사도 통과하므로 조용히 나간다. `pnpm check:md` 가 미렌더 블록을 잡아주니 커밋 전에 같이 돌릴 것.

7. **EN 번역 — 현재 중단 (2026-08-25~).** 작성자가 "다시 켜라" 고 말하기 전까지 `pnpm translate` 를 돌리지 않는다. KO 만 발행하고 커밋한다. 자세한 규칙은 아래 [## 번역 워크플로우 (KO → EN)](#번역-워크플로우-ko--en) 섹션 참고.

8. **Commit & Push**
   - 한 번에 commit: **KO 파일** (번역을 다시 켠 뒤에는 KO + EN 함께) + 이미지 + inbox 업데이트 + 임시 파일 삭제 묶어서
...
3. 포스트 작성 (frontmatter, "공부할 것" 섹션 포함, **다이어그램 트리거 검토**)
4. **🔴 보안 스크럽 필수** — inbox 메모와 동일하게 grep 검사
5. mermaid 블록을 넣었으면 `pnpm mermaid:render` → `pnpm check:md` 로 미렌더 잔존 확인
6. commit & push
...
위 1~5번은 **어떻게 렌더하는가**만 설명한다. 정작 "언제 쓸지" 가 없어서 도구를 만들어놓고 146편 중 3편에서만 썼다. 트리거는 [발행 절차 2번](#절차)에 있다 — 순서 · 계층 · 주체 간 흐름 · 분기가 나오면 다이어그램을 먼저 검토한다.

**영상(Remotion 등)과의 경계**: 영상은 **시간에 따라 변하고 그 변화 자체가 요점일 때**만 쓴다 (예: LED 점멸, PWM 파형). 프로세스 순서나 계층은 정지 그림이 낫다 — 독자가 자기 속도로 훑을 수 있다. 영상은 이미지 최적화 · LCP 와 정면으로 부딪히므로 기본값이 아니다.

### ⚠️ `mermaid:gc` 주의

`--gc` 는 한 번 **사용 중인 SVG 를 전부 삭제한 적이 있다** (2026-08-30). 고아 판정을 살아 있는 ```mermaid``` 블록 기준으로만 해서, 렌더 후 블록이 `<img>` 로 치환되고 나면 모든 SVG 가 고아로 잡혔다. 스크립트는 `<img>` 참조까지 세도록 고쳤지만:

- **고아 경고가 뜨면 먼저 실제 참조를 확인한다** — `grep -ho 'assets/mermaid/[0-9a-f]*\.svg' src/data/blog/*/*.md | sort -u`
- `--gc` 는 **커밋이 깨끗한 상태에서만** 실행한다. 잘못 지워도 `git checkout -- public/assets/mermaid/` 로 복구 가능해야 한다

## 번역 워크플로우 (KO → EN)

> **🛑 자동 번역 중단 중 (2026-08-25~)**
>
...
- ✅ **사용자가 "번역은 나중에"** — 그 세션 skip. 다음에 처리하도록 알림. (2026-08-25~ 는 상시 skip 상태 — 위 배너 참고)
